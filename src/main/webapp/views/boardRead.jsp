...
<title><c:out value="${boardInfo.title}" /></title>
<style>
body {
	background-image:
			...
			<table class="table">
				...
				<tbody>
					<tr>
						...
						<td>
							<textarea style="resize: none; height: 500px;" onfocus="this.blur()" readonly="readonly"  class="form-control" name="content" style="height: 400px; cursor: default"><c:out value="${boardInfo.content}" /></textarea>
						</td>
					</tr>
				</tbody>
			</table>
			<div style="margin: 12px;">
			<a style=" margin:2px; float:right; font-color:white" href="#" onclick="history.back(-1)" class="btn btn-primary">의뢰목록</a> 
			<c:choose>
				<c:when test="${sessionScope.userno == boardInfo.userno }">
				<div class="wrap">
			<a style="margin:2px; float:right;" href="boardDelete?boardno=<c:out value="${boardInfo.boardno}"/>"  class="btn btn-primary">삭제</a>
			
			<a style="margin:2px;  float:right;" href="boardUpdate?boardno=<c:out value="${boardInfo.boardno}"/> "  class="btn btn-primary">수정</a>
			</div>
				</c:when>
			</c:choose>
			</div>
